...
- [Generate an intial boxplot](#generate-an-intial-boxplot)
- [Add some color to the plots](#add-some-color-to-the-plots)
...
Based on the boxplots, we can infer that the “A” allele has the higher
expression with the median value being the highest amongst the three
genotypes for A\|A while G\|G has the lowest expression. The SNP seems
to affect expression as the genotypes containing the “A” allele shows a
higher expression, meaning that the SNP doesn’t affect ORMDL3
expression.
